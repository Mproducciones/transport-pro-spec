// Crear chofer de prueba con token fresco
...
async function createDriverWithFreshToken() {
  console.log('🚛 CREANDO CHOFER DE PRUEBA - TOKEN FRESCO\n');

  // 1. Login como admin con token fresco
  let adminToken = null;
  try {
  ...
  // 2. Verificar que el token funciona obteniendo vehículos
  let vehicles = [];
  try {
    ...
    if (response.statusCode === 200) {
      const data = JSON.parse(response.body);
      vehicles = data.data || [];
      console.log(`✅ Token válido - Vehículos disponibles: ${vehicles.length}`);
    } else {
      console.log('❌ Token inválido - Error obteniendo vehículos:', response.statusCode);
      return;
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.statusCode === 201) {
      ...
      // 4. Probar login inmediato con el nuevo chofer
      ...
      try {
        ...
        if (loginResponse.statusCode === 200) {
          ...
          // 5. Verificar acceso al panel de chofer
          console.log('\n📱 VERIFICANDO ACCESO AL PANEL DE CHOFER:');
          ...
          console.log('✅ Modal de detalle específico para conductores');
        ...
        console.log('❌ Error login con nuevo chofer:', error.message);
      }
